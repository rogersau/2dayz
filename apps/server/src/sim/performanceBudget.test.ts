...
describe("createRoomSimulationConfig", () => {
  it("keeps room budgets inside the v1 player, tick, zombie, and dropped-item limits", () => {
    ...
    expect(() => createRoomSimulationConfig({ sprintSpeedMultiplier: 0.5 })).toThrow(/sprint speed multiplier/i);
    expect(() => createRoomSimulationConfig({ sprintSpeedMultiplier: 1 })).toThrow(/sprint speed multiplier/i);
    expect(() => createRoomSimulationConfig({ sprintSpeedMultiplier: Number.NaN })).toThrow(/sprint speed multiplier/i);
    expect(() => createRoomSimulationConfig({ staminaBaseline: Number.POSITIVE_INFINITY })).toThrow(/stamina baseline/i);
    expect(() => createRoomSimulationConfig({ staminaFloor: Number.NaN })).toThrow(/stamina floor/i);
    expect(() => createRoomSimulationConfig({ staminaDrainPerSecond: Number.POSITIVE_INFINITY })).toThrow(/stamina drain/i);
    expect(() => createRoomSimulationConfig({ staminaRegenPerSecond: Number.NaN })).toThrow(/stamina regen/i);
    expect(() => createRoomSimulationConfig({ staminaLoadPenalty: Number.POSITIVE_INFINITY })).toThrow(/stamina load penalty/i);
