...
const prefixedId = (prefix: string, label: string) => {
  return z.string().regex(new RegExp(`^${prefix}_[a-z0-9-]+$`), `invalid ${label}`);
};
...
export const volumeIdSchema = prefixedId("volume", "volume id");
export const pointIdSchema = prefixedId("point", "point id");
export const zoneIdSchema = prefixedId("zone", "zone id");
export const placementIdSchema = prefixedId("placement", "placement id");
export const nodeIdSchema = prefixedId("node", "node id");
